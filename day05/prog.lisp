...
(defun part1 (file-name)
  (let* ((lines (uiop:read-file-lines file-name))
         ...
         (locations (mapcar (lambda (seed)
                              (let* ((soil (map-input (gethash "seed-to-soil" maps)
                                                      seed))
                                     (fertilizer (map-input (gethash "soil-to-fertilizer" maps)
                                                            soil))
                                     (water (map-input (gethash "fertilizer-to-water" maps)
                                                       fertilizer))
                                     (light (map-input (gethash "water-to-light" maps)
                                                       water))
                                     (temperature (map-input (gethash "light-to-temperature" maps)
                                                             light))
                                     (humidity (map-input (gethash "temperature-to-humidity" maps)
                                                          temperature))
                                     (location (map-input (gethash "humidity-to-loation" maps)
                                                          humidity)))
                                location))
                            seeds)))
    locations))
